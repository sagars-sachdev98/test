...
export default class extends Vue {
  ...
  private async getRoles() {
    const { data } = await getAdminsRoles({
      /* Your params here */
    })
  ...
  private handleDelete(scope: any) {
    ...
    this.$confirm('Confirm to remove the role?', 'Warning', {
      ...
      .then(async() => {
        await deleteAdminsRole(row.key)
  ...
  private async confirmRole() {
    ...
    if (isEdit) {
      await updateAdminsRole(this.role.key, { role: this.role })
      ...
      const { data } = await createAdminsRole({ role: this.role })
