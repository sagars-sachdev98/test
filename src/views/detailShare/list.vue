<template>
  <div class="app-container">
    ...
    >
      ...
      >
        <template slot-scope="scope">
          <span><b>Full Name:</b>{{ scope.row.fullName }}</span><br>
          <span><b>Email:</b>{{ scope.row.email }}</span><br>
          <span><b>Contact Number:</b>{{ scope.row.contactNumber }}</span><br>
          <span><b>Address:</b>{{ scope.row.address }}</span><br>
          <span><b>Occupation:</b>{{ scope.row.occupation }}</span><br>
        </template>
      </el-table-column>
...
import { IDetailShareData } from '@/api/types'
...
import { deleteDetailShare, getDetailShare, updateDetailShare, defaultDetailShareData, getDetailShareById } from '@/api/detailshare'
import { AdminModule } from '@/store/modules/admin'

@Component({
  name: 'DetailShareList',
  components: {
    Pagination,
    Enabled
  }
...
export default class extends Vue {
  ...
  private total = 0;
  private status = ''
  private list: IDetailShareData[] = [];
  private listLoading = true;
  private checked:any[]=[]
  private downloadLoading = false;
  private myRole = AdminModule.roles;
  private postForm: IDetailShareData[] = [];
  private listQuery = {
    page: 1,
    limit: 10,
    sort: 'id,DESC',
  ...
  private handleDelete(data: any) {
    if (data) {
      this.$confirm(
        ...
        .then(() => {
          deleteDetailShare(data.id).then(deleted => {
            this.getList()
          })
        })
        .catch(() => {
  ...
  private async getList() {
    try {
      ...
      const data: any = await getDetailShare(getQuery(this.listQuery))
      this.list = data.data
  ...
  private async updateStatus(id: number, status: any) {
    const data: any = await getDetailShareById(id)
    data.status = status
    await updateDetailShare(id, data)
  }
  ...
  private async handleDownload() {
    ...
    const dataAll:any = await getDetailShare(getQuery(this.listQuery))
    const listAll:IDetailShareData[] = dataAll.data
    ...
    exportJson2Excel(
      tHeader,
      data,
      this.$tc('route.DetailShareManagement.DetailShare.title')
    )
