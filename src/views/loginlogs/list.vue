<template>
  <div class="app-container">
    ...
    >
      ...
      >
        <template slot-scope="scope">
          <router-link :to="'/LoginLog/edit/'+scope.row.id">
            <el-button
              v-waves
              type="primary"
              size="small"
...
import { deleteLoginLog, getLoginLogs, updateLoginLog, defaultLoginLogData, getLoginLogById } from '@/api/loginlogs'
...
export default class extends Vue {
  ...
  private total = 0;
  private status = ''
  private list: ILoginData[] = [];
  private listLoading = true;
  private checked:any[]=[]
  private downloadLoading = false;
  private myRole = AdminModule.roles;
  private postForm: ILoginData[] = [];
  private listQuery = {
    page: 1,
    limit: 10,
    sort: 'id,DESC',
  ...
  private handleDelete(data: any) {
    if (data) {
      this.$confirm(
        ...
        .then(() => {
          deleteLoginLog(data.id).then(deleted => {
            this.getList()
          })
        })
        .catch(() => {
  ...
  private async getList() {
    try {
      ...
      const data: any = await getLoginLogs(getQuery(this.listQuery))
      this.list = data.data
  ...
  private async updateStatus(id: number, status: any) {
    const data: any = await getLoginLogById(id)
    data.status = status
    await updateLoginLog(id, data)
  }
  ...
  private async handleDownload() {
    ...
    const dataAll:any = await getLoginLogs(getQuery(this.listQuery))
    const listAll:ILoginData[] = dataAll.data
    ...
    exportJson2Excel(
      tHeader,
      data,
      this.$tc('route.LoginLogManagement.LoginLog.title')
    )
