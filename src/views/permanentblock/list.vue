<template>
  <div class="app-container">
    ...
    >
      ...
      >
        <template slot-scope="scope">
          <span >{{ scope.row.createdTimestamp | moment("dddd, MMMM Do YYYY, h:mm:ss a") }}</span>
        </template>
      </el-table-column>
      <el-table-column
...
import { IPermanentBlockData } from '@/api/types'
...
import { deletePermanentBlock, getPermanentBlocks, updatePermanentBlock, defaultPermanentBlockData, getPermanentBlockById } from '@/api/permanentblock'
import { AdminModule } from '@/store/modules/admin'
// import { deleteCareer } from '@/api/career'

@Component({
  name: 'PermanentBlockList',
  components: {
    Pagination,
    Enabled
  }
...
export default class extends Vue {
  ...
  private total = 0;
  private status = ''
  private list: IPermanentBlockData[] = [];
  private listLoading = true;
  private checked:any[]=[]
  private downloadLoading = false;
  private myRole = AdminModule.roles;
  private postForm: IPermanentBlockData[] = [];
  private listQuery = {
    page: 1,
    limit: 10,
    sort: 'id,DESC',
  ...
  private handleDelete(data: any) {
    if (data) {
      this.$confirm(
        ...
        .then(() => {
          deletePermanentBlock(data.id).then(deleted => {
            this.getList()
          })
        })
        .catch(() => {
  ...
  private async getList() {
    try {
      ...
      const data: any = await getPermanentBlocks(getQuery(this.listQuery))
      this.list = data.data
  ...
  private async updateStatus(id: number, status: any) {
    ...
    data.status = status
    await updatePermanentBlock(id, data)
  }
  ...
  private async handleDownload() {
    ...
    const dataAll:any = await getPermanentBlocks(getQuery(this.listQuery))
    const listAll:IPermanentBlockData[] = dataAll.data
    ...
    exportJson2Excel(
      tHeader,
      data,
      this.$tc('route.PermanentBlockManagement.PermanentBlock.title')
    )
