<template>
  <div class="app-container">
    ...
    >
      ...
      >
        <template slot-scope="scope">
          <span><b>Title:</b> {{ scope.row.prize.numberOfParticipants }}</span><br>
          <span><b>No of Participants :</b> {{ scope.row.prize.numberOfParticipants }}</span><br>
          <span><b>Per Person Amount :</b> {{ scope.row.prize.perPersonAmount }}</span><br>
          <span><b>No of Participants Paid:</b> {{ scope.row.prize.numberOfParticipantsPaid }}</span><br>
          <span><b>Total Amount Collected:</b> {{ scope.row.prize.totalAmountCollected }}</span><br>
          <span
            v-if="winnerId!=NUll"
            style="background:#ADFF2F;"
          ><b>Winner</b> {{ scope.row.prize.winner.fullName }}</span><br>
        </template>
      </el-table-column>
      ...
      >
        <template slot-scope="scope">
          <span
            v-if="scope.row.scratched == true"
            style="color:green"
          > Yes </span>
          <span
            v-if="scope.row.scratched == false"
            style="color:red"
          > No </span>
        </template>
      </el-table-column>
      <!-- <template slot-scope="scope" >
          <span v-if="scope.row.status == 0" style="color:yellow">
            pending
          </span>
...
import { IPrizeParticipantData } from '@/api/types'
import Pagination from '@/components/Pagination/index.vue'
import { exportJson2Excel } from '@/utils/excel'
import { formatJson, getQuery } from '@/utils'
import Enabled from '@/components/Enabled/index.vue'
import { getPrizeParticipants } from '@/api/prizeparticipant'
import { AdminModule } from '@/store/modules/admin'

@Component({
  name: 'prizeparticipantList',
  components: {
    Pagination,
    Enabled
  }
...
export default class extends Vue {
  ...
  private list: IPrizeParticipantData[] = [];
  ...
  created() {
    this.prizeId = parseInt(this.$route.params && this.$route.params.id)
    this.listQuery.prizeId = this.prizeId
    this.getList()
  }

  //   private fetchWinnerID()
  // {
  //   for (let i=0;i<this.list.length;i++){
  //     if(this.list[i].prize.winnerId!=null)
  //   }
  // }
  ...
  private async getList() {
    try {
      ...
      const data: any = await getPrizeParticipants(getQuery(this.listQuery))
      this.list = data.data
