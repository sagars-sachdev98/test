  ...
  <div class="app-container">
    <div class="filter-container">
      <el-input
        ...
        :placeholder="$t('location.volunteer.fullName')"
        style="width: 200px;"
        class="filter-item"
      ...
      <el-input
        ...
        :placeholder="$t('location.volunteer.contactNumber')"
        style="width: 200px;"
        class="filter-item"
        @keyup.enter.native="handleFilter"
      />
      <el-input
        ...
        :placeholder="$t('location.volunteer.email')"
        style="width: 200px;"
        class="filter-item"
        @keyup.enter.native="handleFilter"
      />
      <el-select
        ...
        :placeholder="$t('location.volunteer.enabled')"
        clearable = true
        class="filter-item"
        style="width: 130px"
        @change="handleFilter"
    ...
    >
      ...
      <el-table-column
        ...
        align="center"
        prop="fullName"
        :label="$t('location.volunteer.fullName')"
      >
        <template slot-scope="scope">
          <enabled
            :key="scope.row.id"
            v-model="scope.row.enabled"
            :url="'/volunteers/' + scope.row.id"
          />
          {{ scope.row.fullName }}
        </template>
      </el-table-column>
      <el-table-column
        ...
        align="center"
        prop="contactNumber"
        :label="$t('location.volunteer.contactNumber')"
      />
      <el-table-column
        width="180px"
        align="center"
        prop="email"
        :label="$t('location.volunteer.email')"
      />
      <el-table-column
      ...
      >
        <template slot-scope="scope">
          <router-link :to="'/volunteers/edit/'+scope.row.id">
            <el-button
              v-waves
              type="primary"
              size="small"
...
export default class extends Vue {
  ...
  private async getList() {
    try {
      ...
      const data: any = await getVolunteers(getQuery(this.listQuery))
      this.list = data.data
  ...
  private handleDelete(data: any) {
    if (data) {
      this.$confirm(
        this.$tc('table.deleteConfirmation', 0, { name: data.fullName }),
        ...
        .then(() => {
          deleteVolunteer(data.id).then(deleted => {
            this.getList()
          })
        })
        .catch(() => {
