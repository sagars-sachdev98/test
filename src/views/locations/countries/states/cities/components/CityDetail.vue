...
import {
  ...
  updateCity,
  createCity
} from '@/api/locations/cities'
import { getStateById } from '@/api/locations/states'
...
export default class extends Vue {
  ...
  created() {
    if (this.isEdit) {
      ...
      this.postForm.stateId = parseInt(
        this.$route.params && this.$route.params.stateId
      )
      // fetch state to get countryId
      this.fetchState(this.postForm.stateId)
    }
  ...
  private async fetchData(id: number) {
    try {
      const data: any = await getCityById(id)
      this.postForm = data
  ...
  private async fetchState(stateId: number) {
    try {
      const stateData: any = await getStateById(stateId)
      this.postForm.countryId = stateData.countryId
    } catch (err) {
      Promise.reject(err)
    }
  }
  ...
  public async saveForm() {
    try {
      ...
      if (this.isEdit) {
        ...
        await updateCity(this.postForm.id, this.postForm)
      } else {
        delete this.postForm.country
        delete this.postForm.state
        await createCity(this.postForm)
      }
