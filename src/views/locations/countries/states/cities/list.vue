  ...
  <div class="app-container">
    <div class="filter-container">
      <el-input
        ...
        :placeholder="$t('location.city.name')"
        style="width: 200px;"
        class="filter-item"
      ...
      <el-select
        ...
        :placeholder="$t('location.city.enabled')"
        clearable = true
        class="filter-item"
        style="width: 130px"
        @change="handleFilter"
...
import { getCities, deleteCity } from '@/api/locations/cities'
...
export default class extends Vue {
  ...
  created() {
    ...
    this.getList(this.stateId)
  }

  private async getList(stateId: number) {
    try {
      this.listLoading = true
      this.listQuery.stateId = stateId
  ...
  private handleDelete(data: any) {
    if (data) {
      this.$confirm(
        ...
        .then(() => {
          deleteCity(data.id).then(deleted => {
            this.getList(this.stateId)
          })
        })
        .catch(() => {
  ...
  private handleDownload() {
    ...
    exportJson2Excel(tHeader, data, this.$tc('route.locations.cities.title'))
