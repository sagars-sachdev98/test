    ...
    >
      ...
      >
        <template slot-scope="scope">
          ...
          <span>{{ scope.row.user.contactNumber }}</span>
        </template>
      </el-table-column>
      ...
      >
        <template slot-scope="scope">
          <span>{{ scope.row.prize.prizeName }}</span>
        </template>
      </el-table-column>
      ...
      >
        <template slot-scope="scope">
          <router-link :to="'/transaction/edit/'+scope.row.id">
            <el-button
              v-waves
              type="primary"
              size="small"
...
export default class extends Vue {
  private tableKey = 0;
  private total = 0;
  private list: IGameTransactionData[] = [];
  private usersList:any = [];
  private plansList:any = [];
  private prizesList:any = [];
  ...
  private async getList() {
    try {
      ...
      const data: any = await getGameTransaction(getQuery(this.listQuery))
      this.list = data.data
  ...
  private async fetchUsers() {
    try {
      this.listLoading = true
      const data: any = await getUsers(getQuery(
        {
          filter: {
          }
        }
  ...
  private async fetchPlans() {
    try {
      this.listLoading = true
      const data: any = await getPlans(getQuery(
        {
          filter: {
          }
        }
   ...
   private async fetchPrizes() {
    try {
      this.listLoading = true
      const data: any = await getPrizes(getQuery(
        {
          filter: {
          }
        }
  ...
  private handleDownload() {
    ...
    exportJson2Excel(
      tHeader,
      data,
      this.$tc('route.transactionManagement.transactions.title')
    )
